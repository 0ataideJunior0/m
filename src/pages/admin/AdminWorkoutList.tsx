    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 dark:from-bg dark:to-bg">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center mb-8">
          <button onClick={() => navigate('/admin/programs')} className="mr-4 p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10 transition">
            <ArrowLeft className="w-6 h-6 text-text" />
          </button>
          <h1 className="text-2xl md:text-3xl font-bold text-text">{program?.name}</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {days.map(({ weekdayNumber, workout }) => (
            ...
            >
              ...
              {workout ? (
                <>
                  <div className="text-lg font-bold text-text mb-1">{workout.title}</div>
                  <div className="text-sm text-text-muted">{workout.exercises?.length || 0} exercícios</div>
                </>
              ) : (
                <div className="text-lg font-bold text-purple-600 dark:text-purple-400 mb-1">+ Criar treino</div>
              )}
            </Link>
          ))}
        </div>
